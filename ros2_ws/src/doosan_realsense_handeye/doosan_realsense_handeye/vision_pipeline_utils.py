...
def detect_books(
    ...
    yolo_conf=None,
    display_conf_threshold=None,
):
    if yolo_conf is None:
        yolo_conf = vision.YOLO_CONF
    if display_conf_threshold is None:
        display_conf_threshold = float(yolo_conf)
    ...
    for index, obb in enumerate(yolo_results[0].obb):
        ...
        if confidence < float(display_conf_threshold):
            continue
