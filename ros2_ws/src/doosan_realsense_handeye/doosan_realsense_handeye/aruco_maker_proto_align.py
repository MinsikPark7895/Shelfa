...
class ArucoMarkerProtoAlign(Node):
    def __init__(self):
        ...
        self.declare_parameter("auto_step_period_sec", 0.3)        # 변경: 2.0 -> 0.3
        self.declare_parameter("auto_post_motion_wait_sec", 0.5)   # 변경: 5.0 -> 0.5
        self.declare_parameter("auto_tf_retry_sec", 0.2)           # 변경: 1.0 -> 0.2
        ...
        self.declare_parameter("max_rot_step_deg", 3.0)            # 변경: 1.0 -> 3.0
        self.declare_parameter("sign_tool_b_from_camera_y", 1.0)
        self.declare_parameter("rot_vel_linear", 30.0)         # 변경: 10.0 -> 30.0
        self.declare_parameter("rot_vel_angular", 15.0)       # 변경: 5.0 -> 15.0
        self.declare_parameter("rot_acc_linear", 60.0)         # 변경: 20.0 -> 60.0
        self.declare_parameter("rot_acc_angular", 30.0)       # 변경: 10.0 -> 30.0
        ...
        self.declare_parameter("coarse_max_step_mm", 100.0)        # 신규: coarse 전용 최대 이동량
        ...
        self.declare_parameter("max_step_mm", 10.0)                # 변경: 1.0 -> 10.0 (fine 전용)
        ...
        self.declare_parameter("trans_vel_linear", 40.0)     # 변경: 10.0 -> 40.0
        self.declare_parameter("trans_vel_angular", 10.0)
        self.declare_parameter("trans_acc_linear", 80.0)     # 변경: 20.0 -> 80.0
        self.declare_parameter("trans_acc_angular", 20.0)
        self.declare_parameter("recheck_rotation_after_translation", True)
    ...
    def run_translation_step(
        ...
        # label에 따라 coarse/fine max_step_mm 선택
        if label == "coarse translation":
    ...
    def compute_translation_step(
        ...
        max_step_mm: 호출 측에서 coarse/fine 구분하여 전달.
          - coarse translation -> self.coarse_max_step_mm (기본 100.0 mm)
          - fine   translation -> self.max_step_mm        (기본  10.0 mm)
        """
    ...
    def read_current_tcp_posx(self):
        if self.dry_run:
            self.get_logger().warn(
                "dry_run=true: current TCP pose is not read for alignment payload."
            )
            return None
