...
from dsr_example.simple.gripper_tcp_protocol import (
    Command,
    GripperState,
    StatusCode,
    build_packet,
...
class DoosanGripperTcpBridge:
    ...
    def start(self) -> None:
        ...
        if current_state == DRL_PROGRAM_STATE_PLAY:
            self._node.get_logger().warning(
                "A DRL program is already running. Starting the gripper TCP server may fail."
            )
        ...
        if response is None or not response.success:
            raise RuntimeError("Failed to start the DRL gripper TCP server.")
    ...
    def initialize(self, goal_current: int | None = None) -> GripperState:
        current = self._config.goal_current if goal_current is None else int(goal_current)
        return self._request_state(Command.INITIALIZE, pack_initialize_payload(current))
    ...
    def _connect_tcp_client(self) -> None:
        ...
        raise RuntimeError(f"Failed to connect to controller TCP server: {last_error}")
    ...
    def _build_drl_server_script(self) -> str:
        ...
        return textwrap.dedent(
            ...
            while initialize_gripper() is False:
                wait(0.5)
