...
function Register() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const allUsers = await fetch('http://localhost:3001/users').then(r => r.json())
      const memberNo = `${year}-${String(allUsers.length + 1).padStart(4, '0')}-${String(Math.floor(Math.random() * 10000)).padStart(4, '0')}`
