...
`dev_vision` 브랜치는 책등 인식, OCR, RealSense Depth, ArUco TF 검출을 담당하는 `vision/` 모듈을 포함합니다. 로봇 모션 로직과 직접 결합하기보다 JSON 결과와 TF 기준 데이터를 통해 다른 ROS 2 노드가 소비할 수 있게 구성합니다.

Main contents:

- `vision/realtime_yolo_paddle_ocr.py`: RealSense + YOLO-OBB + PaddleOCR 실시간 파이프라인
- `vision/book_scan_after_alignment.py`: ArUco 정렬 payload 이후 책 스캔/OCR 결과 생성
- `vision/vision_pipeline_utils.py`: OBB 후처리, keypoint, depth helper
- `vision/aruco_realsense_tf_publisher.py`: RealSense 컬러 영상 기반 ArUco TF publisher
- `vision/weights/best.pt`: 책등 YOLO-OBB 모델 가중치

Runtime outputs such as `realtime_results/`, OCR debug crops, `__pycache__/`, ROS build outputs, and local IDE indexes should not be committed.
